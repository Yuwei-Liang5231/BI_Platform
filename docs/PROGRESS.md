...
| 统一取数 | `POST /api/query/metric-value`（body: metric=id或code / start / end / compare=none·mom·yoy）→ `{metric_id, metric_code, name, ver, start, end, value, period_complete, cache, coverage, grain, compare}`；`POST /api/query/export`（同参数）→ CSV 文件流（**契约例外：不走 JSON envelope**，UTF-8 BOM，Content-Disposition 文件名 `{code}_{start}_{end}.csv`）；metric 入参兼容 id 与 code | B3 |
| 周期完整性语义 | 请求范围超出 `dataset_coverage` 登记区间 → `value=null` 且 `period_complete=false`（前端显示"—"），**不缓存**（补录数据后结果会变）；完整周期但区间无数据 / 比率分母 0 → `value=null` 且 `period_complete=true`；两种 null 语义靠标志区分 | B3 |
| 缓存语义 | 进程内 LRU（512 条，线程安全，deepcopy 隔离）；键 = `metric:指标id:ver:数据集名:dataset_ver(有序):start:end:sha1(sql)[:12]`；**不完整周期不入缓存**；口径变更（ver+1）或数据重导入（dataset_ver+1）→ 旧键自然失效；多 worker 部署时仅需替换 `app/infra/cache.py` 实现 | B3 |
...
- **MVP（阶段 0+1）已全部完成**。后续方向需另行评估排期：B8 增量导入、自然语言问数（LLM 口径草稿）、行级权限（4b）、扩展算子（4a）。
