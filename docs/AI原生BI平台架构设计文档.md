...
| **D19** | **指标涨跌配色语义（V1.2 新增，V1.3 已确认，2026-09-15 用户修订）** | **红涨绿跌**：涨 = Red `#D62222`（PwC Error 500，令牌 `--pwc-up`），跌 = Green `#059669`（PwC Success 500，令牌 `--pwc-down`），图标统一用**三角**（▲/▼，不再叠加箭头），配文字（不依赖颜色单通道传达信息） | 用户明确要求改为中国惯例红涨绿跌并只保留三角；颜色按**变化方向**编码（与 good/bad 业务语义解耦，比率类指标 goodWhenUp=false 时颜色仍随涨跌）；深色主题用 300 档保证对比度；保留文字作为无障碍冗余 |
